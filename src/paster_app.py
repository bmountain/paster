...
from .data_model import Config, Children, Child
...
class ExcelPaster:
    ...
    def __init__(self, config: Config, children: Children):
        self.row = 1
        self.col = 1
        self.dir_idx = 0
        self.wb = Workbook()
    ...
    def write_one_dir(self, child: Child) -> None:
        ...
        self.row += self.config.row_gap_small + 1
        ...
        for i, img in enumerate(img_paths):
            ...
            if i == len(img_paths) - 1:  # その項番の最後の画像の場合
                self.row += max(height_per_row_list) + self.config.row_gap_big
                self.col = 1
            elif i % n_cols == n_cols - 1:  # 改行するがその項番の最後の画像ではない場合
                self.row += max(height_per_row_list) + self.config.row_gap_small
                self.col = 1
                height_per_row_list = []
            else:  # 改行しない場合
                self.col += width_per_col + self.config.col_gap
        self.col = 1
    ...
    def resize(self, img: Path) -> Image:
        ...
        height = min(
            ...
            img.height * self.config.max_width / img.width,
        )
    ...
    def get_img_size_in_cell(self, img: Image) -> tuple[int]:
        ...
        height_per_row = math.ceil(
            pixels_to_points(img.height, dpi=self.config.dpi)
            / self.config.row_height_point
        )
